...
.am-grid .am-flexbox .am-flexbox-item .am-grid-item-content .am-grid-item-inner-content .error {
  color: red;
  font-size: 24px;
}
...
.am-list-body :not(:last-child) .am-list-line {
  border-bottom: 1px solid #ddd;
}
@media (min-resolution: 2dppx) {
  html:not([data-scale]) .am-list-body :not(:last-child) .am-list-line {
    border-bottom: none;
  }
  html:not([data-scale]) .am-list-body :not(:last-child) .am-list-line::after {
    content: '';
    position: absolute;
    background-color: #ddd;
    display: block;
